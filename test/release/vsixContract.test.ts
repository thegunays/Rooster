...
function validVsixManifest(version = "0.1.25"): string {
...
describe("effective packaged metadata and payload bytes", () => {
  ...
  it.each([
    ["version", validVsixManifest("0.1.26")],
  ...
  it("rejects a symlinked derived artifacts directory during standalone verification", () => {
    ...
    const artifactName = "ygdb121.roosterjs-description-editor-0.1.25.vsix";
...
`);
  ...
  it("invokes only the lock-resolved local tfx entry and transactionally verifies its archive", () => {
    ...
    const artifact = join(
      fixture.root,
      "artifacts/ygdb121.roosterjs-description-editor-0.1.25.vsix"
    );
    ...
    expect(readdirSync(join(fixture.root, "artifacts"))).toEqual([
      "ygdb121.roosterjs-description-editor-0.1.25.vsix"
    ]);
