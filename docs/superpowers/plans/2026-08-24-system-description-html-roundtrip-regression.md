...
**Scope:** Diagnose and fix only the rendering regressions documented in `RossterBulgular.pdf`. Keep the existing `FieldName=System.Description` configuration, extension manifest shape, dependencies, telemetry, and UI unchanged. Package the fix as version `0.1.24` because `0.1.23` is already an issued artifact.
...
- [x] Confirm the baseline package and manifest version `0.1.23`, then reserve `0.1.24` for this fix.
...
- [x] Confirm the resulting VSIX is version `0.1.24` and record its checksum.
...
- All repository verification gates pass and the artifact version is `0.1.24`.
...
- Artifact: `ygdb121.roosterjs-description-editor-0.1.24.vsix`
- SHA-256: `6e89b539850a4fc2557138eaf7dbff0516cfb6d1cfba1666e55d8090635e17ba`
